...
SocketTCP.event = require("cocos.framework.components.event")
...
--	cc(self):addComponent("fffcomponents.event"):exportMethods()
...
function SocketTCP:close( ... )
	...
	self.event:dispatchEvent({name=SocketTCP.EVENT_CLOSE})
end
...
function SocketTCP:_disconnect()
	self.isConnected = false
	self.tcp:shutdown()
	self.event:dispatchEvent({name=SocketTCP.EVENT_CLOSED})
end
...
function SocketTCP:_onConnected()
	...
	self.event:dispatchEvent({name=SocketTCP.EVENT_CONNECTED})
	if self.connectTimeTickScheduler then scheduler:unscheduleScriptEntry(self.connectTimeTickScheduler) end
	...
	local __tick = function()
		while true do
			...
			self.event:dispatchEvent({name=SocketTCP.EVENT_DATA, data=(__partial or __body), partial=__partial, body=__body})
		end
	end
...
function SocketTCP:_connectFailure(status)
	...
	self.event:dispatchEvent({name=SocketTCP.EVENT_CONNECT_FAILURE})
	self:_reconnect();
end
